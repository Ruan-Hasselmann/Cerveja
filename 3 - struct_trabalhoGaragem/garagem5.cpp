...
string pegaHoraString() {
	...
	hora << horaLocal->tm_hour << ":" << horaLocal->tm_min;
...
void atualizaArquivoComCarros(Vaga garagem[], int n) {
	ofstream procuradorEscrita;
	procuradorEscrita.open(NOMEARQUIVO);
	
	for (int i = 0; i < n; i++) {
		if (garagem[i].placa != "") {
			procuradorEscrita << garagem[i].placa 
			     << " " << garagem[i].dataEntrada 
				 << " " << garagem[i].horaEntrada << endl;
		}
	}
...
void entraCarroGaragem(Vaga garagem[], int n) {
	...
	//da garagem
	atualizaArquivoComCarros(garagem, n);
